                    ...
                    <table id="example" class="table table-striped" style="width:100%">
                        ...
                        <tbody>
                            ...
                            while ($r = mysqli_fetch_assoc($rows)) {
                                $assetid = $r['assetid'];
                                $suspendButton = '<button class="btn btn-danger" onclick="action(\''.$r['assetid'].'\', \'active\', 0)">Suspend</button>';
                                $activateButton = '<button class="btn btn-success" onclick="action(\''.$r['assetid'].'\', \'active\', 1)">Activate</button>';
                                $visits = mysqli_fetch_assoc(mysqli_query($con, "SELECT visits FROM visits WHERE assetid='".$assetid."'"))['visits'];
                                $sold = mysqli_num_rows(mysqli_query($con, "SELECT * FROM transactions WHERE assetid='$assetid'"));
                                $jammed = mysqli_num_rows(mysqli_query($con, "SELECT * FROM transactions WHERE locked=0 AND assetid='$assetid'"));
                                $authorid = $r['authorid'];
                            ...
                            <tr>
                                ...
                                <td><a href="transactions?id=<?php echo $authorid;?>"><?php echo $r['price']." ".$r['currency']; ?></a></td>
                                <td><?php if($visits==""){echo "0";}else{echo $visits;} ?></td>
                                <td><?php echo $r['qty']; ?></td>
                                <td><?php echo $sold; ?></td>
                                <td><?php echo $jammed; ?></td>
                                <td><?php echo ($sold/$r['qty'])*100; ?>%</td>
                                <td><?php echo ($r['status'] == 1) ? $suspendButton : $activateButton; ?> <a href="edit?id=<?php echo $r['assetid'];?>" class="btn btn-primary"><i class="fa fa-pen"></i></a></td>
                            </tr>
                            <?php } ?>
                        </tbody>
                    </table>
