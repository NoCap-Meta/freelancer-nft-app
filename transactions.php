    ...
    <div class="container main-container">
        <div class="row">
            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                <h4>Transactions
                    <?php if($filter) {echo '('.$row['name'].')';} ?>
                </h4>
            </div>
            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 wallet-but">
                <button class="btn btn-primary" onclick="location.href='create'" style="width:200px"><i class="fa fa-plus"></i> Create a Listing</button>
            </div>
        </div><br><Br>
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                    <table id="example" class="table table-striped" style="width:100%">
                        ...
                        <tbody>
                            <?php 
                            if ($filter) {
                                $rows = mysqli_query($con, "SELECT * FROM transactions t WHERE t.assetid IN (SELECT assetid FROM assets WHERE authorid='$authorid')");
                            } else {
                                $rows = mysqli_query($con, "SELECT * FROM transactions t");
                            }

                            while ($r = mysqli_fetch_assoc($rows)) {
                                $suspendButton = '<button class="btn btn-danger" onclick="action(\''.$r['orderid'].'\', \'active\', 0)">Suspend</button>';
                                $activateButton = '<button class="btn btn-success" onclick="action(\''.$r['orderid'].'\', \'active\', 1)">Activate</button>';
                                $visits = mysqli_fetch_assoc(mysqli_query($con, "SELECT visits FROM visits WHERE assetid='".$r['assetid']."'"))['visits'];
                                $assetid = $r['assetid'];
                                $asset = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM assets WHERE assetid='$assetid'"));
                            ?>
                            <tr>
                                ...
                                <td><?php if($r['locked']==1){echo "Yes";}else{echo "Jammed";} ?></td>                                
